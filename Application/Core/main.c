...
#define RTOS_DTM_SECTION_SIZE 50 * 1024
#define BK_TASK_STACK_SIZE 500
#define MAIN_TASK_STACK_SIZE 700
#define SD_TASK_STACK_SIZE 350
...
#if defined ( __GNUC__ ) /*!< GNU Compiler */
char tmpstr[256] __attribute__((section(".DTCMSection")));
#else
char tmpstr[256];
#endif
...
void Bk_Task(void const *args) 
{
 ...
 /* Init Host Library */
  ...
  while (1) 
  {
    ...
    // sprintf(tmpstr, "Number of Drives : %d", FATFS_GetAttachedDriversNbr());
    // __log(tmpstr);

    // for (int drive_no = 0; drive_no < FATFS_GetAttachedDriversNbr(); ++drive_no)
    // {
    //   sprintf(tmpstr, "Drive %d : %d", drive_no, disk.drv[drive_no]);
    //   __log(tmpstr);
    // }

    // sprintf(tmpstr, "SD : %s // USB : %s", SDPath, USBDISKPath);
    // __log(tmpstr);
